...
class TierFourCommands(Cog):
    ...
    async def mem_warn(self,inter,member:disnake.Member,*,reason:str):
        ...
        log_channel = self.bot.get_channel(data["log_channels"]["warnings"])
    ...
    async def mem_mute(self,inter,member:disnake.Member,time:int,*,reason:str):
        ...
        mute_role = disnake.utils.get(self.bot.get_guild(guild_id).roles,name="Muted")
        log_channel = self.bot.get_channel(data["log_channels"]["mutes"])
