...
class TierTwoCommands(Cog):
    ...
    async def mem_ban(self, inter, member: disnake.Member, time: int, *, reason: str):
        ...
        ban_role = disnake.utils.get(
            self.bot.get_guild(guild_id).roles, name="Banned")
        log_channel = self.bot.get_channel(data["log_channels"]["bans"])
    ...
    @commands.slash_command(
        ...
        description="Updates Database To Insert Non-Existent Members"
    )
    ...
    @commands.slash_command(
        name="purge",
        description="Deletes _N_ Messages From The Channel Executed In"
    )
    ...
    async def purge_message(self, inter, num: int, *, reason: str):
        if num > 25:
            return await inter.response.send_message("Do Not Purge More Than 50 Messages At A Time, {}".format(inter.author.mention), ephemeral=True)
        else:
            ...
            embed = disnake.Embed(
                ...
                timestamp=inter.created_at,
                title="{}'s Purge Messages Command".format(
            ...
            await inter.edit_original_message(f"Purging Of {num} Messages Complete")
            log_channel = self.bot.get_channel(
                data["log_channels"]["channel_deletes"])
            await log_channel.send(embed=embed)
